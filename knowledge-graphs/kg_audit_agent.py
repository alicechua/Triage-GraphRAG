...
def _extract_triads(G) -> list[dict]:
    ...
    Each triad is a dict with keys:
        finding, condition (str|None), test, source, rule_out, cache_key
    """
...
def _semantic_scholar_search(query: str) -> tuple[list[dict], int]:
    """Search Semantic Scholar.

    Returns (papers, total) where papers contains up to 100 results (API max)
    with title/year/abstract, and total is the full corpus count from the API.
    """
    headers = {"x-api-key": _SS_API_KEY} if _SS_API_KEY else {}
    try:
        resp = requests.get(
            f"{_SS_BASE}/paper/search",
            params={"query": query, "fields": "title,year,abstract", "limit": 100},
            headers=headers,
            timeout=15,
        )
        ...
        # Sort by year descending so mean_year reflects the most recent literature
        papers.sort(key=lambda p: p.get("year") or 0, reverse=True)
        return papers, body.get("total", 0)
    except Exception:
        return [], 0
...
def _persona_b(triad: dict) -> dict:
    ...
    papers, total_count = _semantic_scholar_search(query)
...
def main() -> None:
    ...
    print(f"  {len(triads)} unique triads to audit")
