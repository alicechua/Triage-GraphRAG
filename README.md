...
│   │   ├── build_vector_db_chroma.py   # ChromaDB + MedEmbed local indexer
│   │   └── embed_kg_nodes.py           # KG node embedder for Qdrant
│   └── requirements.txt
│
├── data-processing/                    # Standalone CSV diagnosis-mapping tools
│   ├── kg_substring_csv_mapping.py             # Substring-based dx → KG node → tests mapping
│   ├── graphrag_semantic_csv_mapping.py        # Semantic mapping via Qdrant + Nebius embeddings
│   └── graphrag_semantic_csv_mapping_chroma.py # Semantic mapping via ChromaDB + MedEmbed (local)
│
...
|---|---|---|
| **ClinGraph** | Bundled in `data/clingraph_data/` | Structured clinical graph data for KG enrichment (`enrich_from_clingraph.py`) |
